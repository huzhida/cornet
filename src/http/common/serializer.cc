...
void serializer_t::request_line(out_buffer_t& out, method_t m, std::string_view path,
                                std::string_view query, version_t v) {
  out.put(method_name(m));
  out.put_byte(' ');
  if (path.empty()) {
    out.put_byte('/');
  } else {
  ...
  // the version and the CRLF are one constant, so this is a single memcpy
  if (v == version_t::Http10) {
    out.put(" HTTP/1.0\r\n", 11);
  } else {
    out.put(" HTTP/1.1\r\n", 11);
  }
