...
expected<void> body_buffer_t::grow(buffer_pool_t& pool, uint32_t bytes) {
  if (bytes <= cap_) return {};
  auto lease = pool.acquire(bytes);
  if (!lease) return unexpected(ENOMEM);
  if (size_ > 0) {
    std::memcpy(lease.data(), lease_.data(), size_);
  }
  lease_ = std::move(lease);
  // the pool rounds up to a class size; take the room it actually handed over
