...
shutdown_awaiter::shutdown_awaiter(int fd, int how) : fd_(fd), how_(how) {
  this->ctx = &context_t::current();
  this->prepare_fn = [](utask_t* self, io_uring_sqe* sqe) {
...
openat_awaiter::openat_awaiter(int dirfd, const char* path, int flags, mode_t mode)
...
splice_awaiter::splice_awaiter(int fd_in, int64_t off_in, int fd_out, int64_t off_out,
...
poll_add_awaiter::poll_add_awaiter(int fd, unsigned int poll_mask) : fd_(fd), poll_mask_(poll_mask) {
  this->ctx = &context_t::current();
  this->prepare_fn = [](utask_t* self, io_uring_sqe* sqe) {
...
statx_awaiter::statx_awaiter(int dirfd, const char* path, int flags, unsigned int mask, struct statx* stx)
...
unlinkat_awaiter::unlinkat_awaiter(int dirfd, const char* path, int flags)
...
renameat_awaiter::renameat_awaiter(int old_dirfd, const char* old_path, int new_dirfd, const char* new_path, unsigned int flags)
...
mkdirat_awaiter::mkdirat_awaiter(int dirfd, const char* path, mode_t mode)
...
fsync_awaiter::fsync_awaiter(int fd, unsigned int flags) : fd_(fd), flags_(flags) {
  this->ctx = &context_t::current();
  this->prepare_fn = [](utask_t* self, io_uring_sqe* sqe) {
...
coro_t<expected<size_t>> splice_forward(int fd_in, int fd_out, size_t chunk_size) {
  int pipefd[2];
  if (::pipe(pipefd) < 0) {
  ...
  size_t total = 0;
  for (;;) {
    auto n = co_await splice_awaiter(fd_in, -1, pipefd[1], -1, chunk_size, SPLICE_F_MOVE);
    if (!n) {
      ::close(pipefd[0]);
      ::close(pipefd[1]);
    ...
    int remaining = *n;
    while (remaining > 0) {
      auto w = co_await splice_awaiter(pipefd[0], -1, fd_out, -1, remaining, SPLICE_F_MOVE);
      if (!w) {
        ::close(pipefd[0]);
        ::close(pipefd[1]);
