#ifndef CORNET_HTTP_COMMON_SERIALIZER_H
#define CORNET_HTTP_COMMON_SERIALIZER_H
...
/**
 * @brief writes request lines, status lines and headers into an out_buffer_t.
 *
...
class serializer_t {
 public:
  /**
   * @brief write "METHOD /path?query HTTP/1.1\r\n".
   *
   * Path and query are separate parameters rather than one target view because they
   * are not one contiguous run in a url: "http://host?x=1" has a query and no path,
   * and the wire form still needs the "/" between them.
   * @param path path as written; "/" is emitted when it is empty
   * @param query query without the '?'; skipped when empty
   */
  static void request_line(out_buffer_t& out, method_t m, std::string_view path,
                           std::string_view query = {}, version_t v = version_t::Http11);
...
#endif // CORNET_HTTP_COMMON_SERIALIZER_H
