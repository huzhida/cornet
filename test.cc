...
public:
    ...
    BenchmarkResult run(const BenchmarkConfig& config) {
        ...
        if (clients_per_thread == 0) clients_per_thread = 1;
...
int main(int argc, char* argv[]) {
    try {
        ...
        if (argc > 1) config.num_connections = std::stoi(argv[1]);
        if (argc > 2) config.message_size = std::stoi(argv[2]);
        if (argc > 3) config.total_messages = std::stoi(argv[3]);
        ...
        if (results.size() == 2) {
            std::cout << "\n\n========== 性能对比 ==========\n";
            std::cout << std::left << std::setw(15) << "指标"
                      << std::right << std::setw(15) << "Cornet"
                      << std::right << std::setw(15) << "ASIO"
                      << std::right << std::setw(15) << "Cornet/ASIO\n";
            std::cout << "--------------------------------------------------------\n";

            auto& cornet = results[0];
            auto& asio = results[1];

            std::cout << std::left << std::setw(15) << "RPS"
                      << std::right << std::setw(15) << std::fixed << std::setprecision(2)
                      << cornet.requests_per_second
                      << std::right << std::setw(15) << asio.requests_per_second
                      << std::right << std::setw(15)
                      << (cornet.requests_per_second / asio.requests_per_second * 100) << "%\n";

            std::cout << std::left << std::setw(15) << "吞吐量(MB/s)"
                      << std::right << std::setw(15) << cornet.throughput_mbps
                      << std::right << std::setw(15) << asio.throughput_mbps
                      << std::right << std::setw(15)
                      << (cornet.throughput_mbps / asio.throughput_mbps * 100) << "%\n";

            std::cout << std::left << std::setw(15) << "平均延迟(μs)"
                      << std::right << std::setw(15) << cornet.avg_latency_us
                      << std::right << std::setw(15) << asio.avg_latency_us
                      << std::right << std::setw(15)
                      << (asio.avg_latency_us / cornet.avg_latency_us * 100) << "%\n";

            std::cout << std::left << std::setw(15) << "P95延迟(μs)"
                      << std::right << std::setw(15) << cornet.p95_latency_us
                      << std::right << std::setw(15) << asio.p95_latency_us
                      << std::right << std::setw(15)
                      << (asio.p95_latency_us / cornet.p95_latency_us * 100) << "%\n";
        }
